...
for (j in 1:WR){
  ...
  for (i in 1:7){
    ...
    precopula[,i] = pdist(distribution = "std", residuals[,i], mu = 0, sigma = 1 ,shape = paramsstd[i,3])
  }
  RMVC = RVineStructureSelect(precopula,type="CVine")
  simRVC = RVineSim(M, RMVC)
  for (k in 1:7){
    newresiduals[,k] = qdist(distribution = "std",  simRVC[,k], mu = 0, sigma = 1, shape = paramsstd[k,3])
...
write.table(data.frame(VaR),"VaRGASCVine_std.txt",sep=",",dec=".",col.names=FALSE,row.names = FALSE)
write.table(data.frame(ES),"ESGASCVine_std.txt",sep=",",dec=".",col.names=FALSE,row.names = FALSE)
write.table(data.frame(obport),"obport.txt",sep=",",dec=".",col.names=FALSE,row.names = FALSE)
