...
paraminnov = matrix(0,nrow=7,ncol=4)
...
for (j in 1:WR){
  ...
  for (i in 1:7){
    ...
    paraminnov[i,] = rugarch::fitdist(distribution = "sged", residuals[,i])$pars
    precopula[,i] = pdist(distribution = "sged", residuals[,i], mu = paraminnov[i,1], sigma = paraminnov[i,2] ,shape = paraminnov[i,4], skew = paraminnov[i,3])
  }
  RMVC = RVineStructureSelect(precopula,type="CVine")
  simRVC = RVineSim(M, RMVC)
  for (k in 1:7){
    newresiduals[,k] = qdist(distribution = "sged",  simRVC[,k], mu = paraminnov[k,1], sigma = paraminnov[k,2], shape = paraminnov[k,4], skew = paraminnov[k,3])
...
write.table(data.frame(VaR),"VaRRGARCH2CVine_sged.txt",sep=",",dec=".",col.names=FALSE,row.names = FALSE)
write.table(data.frame(ES),"ESRGARCH2CVine_sged.txt",sep=",",dec=".",col.names=FALSE,row.names = FALSE)
