...
for (j in 1:WR){
  ...
  for (i in 1:7){
    ...
    paramsstd[i,] = rugarch::fitdist(distribution = "sstd", residuals[,i])$pars  # ast1 and sstd both are skew student-t distributions
    precopula[,i] = pdist(distribution = "sstd", residuals[,i], mu = paramsstd[i,1], sigma = paramsstd[i,2], shape = paramsstd[i,4], skew = paramsstd[i,3])
  }
  RMVC = RVineStructureSelect(precopula,type="RVine")
  simRVC = RVineSim(M, RMVC)
  for (k in 1:7){
    newresiduals[,k] = qdist(distribution = "sstd",  simRVC[,k], mu = paramsstd[k,1], sigma = paramsstd[k,2], shape = paramsstd[k,4], skew = paramsstd[k,3])
...
write.table(data.frame(VaR),"VaRGASRVine2_sstd.txt",sep=",",dec=".",col.names=FALSE,row.names = FALSE)
write.table(data.frame(ES),"ESGASRVine2_sstd.txt",sep=",",dec=".",col.names=FALSE,row.names = FALSE)
